...
import { app, games } from './server-app.js';
...
describe('Tic Tac Toe API', () => {
  ...
  describe('POST /api/games/:id/move', () => {
    ...
    it('should return 400 when game is already over', async () => {
      ...
      await request(app).post(`/api/games/${gameId}/move`).send({ position: 0 }); // X
      await request(app).post(`/api/games/${gameId}/move`).send({ position: 3 }); // O
      await request(app).post(`/api/games/${gameId}/move`).send({ position: 1 }); // X
      await request(app).post(`/api/games/${gameId}/move`).send({ position: 4 }); // O
      await request(app).post(`/api/games/${gameId}/move`).send({ position: 2 }); // X wins
